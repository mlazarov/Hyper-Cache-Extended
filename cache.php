...
if ($hyper_qs !== false) {
    if ($hyper_cache['strip_qs']) $hyper_uri = substr($hyper_uri, 0, $hyper_qs);
    else if (!$hyper_cache['cache_qs']) return false;
}
...
// If it's required to bypass the cache when the visitor is a commenter, stop.
    if ($hyper_cache['comment'] && substr($n, 0, 15) == 'comment_author_') return false;
...
function hyper_cache_callback($buffer) {
    ...
    if (!$hyper_cache['charset']) $hyper_cache['charset'] = 'UTF-8';
...
function hyper_mobile_type() {
    ...
    if (!isset($hyper_cache['mobile']) || $hyper_cache['mobile_agents'] === false) return '';
